...
const cadreApi = {
  ...
  getInformasi: () => cadreApi.fetchWithError("/api/cadre/dashboard?action=informasi"),
  createInformasi: (formData: FormData) =>
    cadreApi.fetchWithError("/api/cadre/dashboard", {
      method: "POST",
      body: formData,
    }),
...
export default function MobileCadreInformasiPage() {
  ...
  const loadInformasi = async () => {
    const data = await cadreApi.getInformasi();
  ...
  const submitInformasi = async () => {
    ...
    try {
      ...
      if (editingId) {
        await cadreApi.updateInformasi(formData);
      } else {
        await cadreApi.createInformasi(formData);
      }
      setJudul("");
  ...
  const removeInformasi = async (id: string) => {
    try {
      ...
      await cadreApi.deleteInformasi(formData);
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>Kelola pengumuman kegiatan, penyuluhan, dan edukasi gizi.</p>
      </header>
      ...
      <main className={styles.main}>
        <section className={styles.card}>
          ...
          <div className={styles.form}>
            ...
            <input className={styles.input} value={judul} onChange={(e) => setJudul(e.target.value)} placeholder="Contoh: Penyuluhan MPASI Seimbang" />
            ...
            <input className={styles.input} value={lokasi} onChange={(e) => setLokasi(e.target.value)} placeholder="Balai Posyandu RT 03" />

            <label className={styles.label}>Konten</label>
            <textarea className={styles.textarea} value={konten} onChange={(e) => setKonten(e.target.value)} placeholder="Isi informasi untuk orang tua..." />
